 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- needed for recursive keypath traversal
function get<T = unknown>(data: Record<string | number | symbol, any>, keys: string | string[]): T | undefined {
 ...
 * @param keys - keypath to a value
 * @param value - new value to assign at the keypath
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- needed for recursive keypath traversal
function set<T = unknown>(data: Record<string | number | symbol, any>, keys: string | string[], value: T): void {
  const parsedKeys = Array.isArray(keys) ? keys : keys.split('.');
  const key = parsedKeys.shift();
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- needed for recursive keypath traversal
function has(data: Record<string | number | symbol, any>, keys: string | string[]): boolean {
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- needed for recursive keypath traversal
function insert<T = unknown>(data: Record<string | number | symbol, any>, keys: string | string[], value: T): void {
  const parsedKeys = Array.isArray(keys) ? [...keys] : keys.split('.');
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- needed for generic array access
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- needed for recursive keypath traversal
function remove(data: Record<string | number | symbol, any>, keys: string | string[]): void {
  const parsedKeys = Array.isArray(keys) ? [...keys] : keys.split('.');
  ...
  const lastKey = parsedKeys.pop()!;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- needed for generic object access
 ...
 */
function renumberKeys(keys: string[]): Map<string, string> {
...
export { get, set, has, insert, remove, renumberKeys };
