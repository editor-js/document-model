...
export default defineConfig({
  ...
  forbidOnly: isCI,
  retries: isCI ? 2 : 0,
  // On CI the html report is written too, for the e2e action to upload: it is what keeps the
  // traces recorded on a retry (see `trace` below) once the runner is gone.
