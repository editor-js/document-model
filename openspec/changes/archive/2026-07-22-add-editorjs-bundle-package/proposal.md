...
- Introduce a new `@editorjs/editorjs` workspace package: a batteries-included entry point that wires `Core` + `DOMAdapters` + `CollaborationManager` + default tools (paragraph, bold, italic, link) + plugins (clipboard, shortcuts) + UI, auto-initializes in its constructor, and exposes an `isReady` promise.
- `@editorjs/editorjs` honors a v2-style `config.tools` map, merged on top of the defaults with override-by-name.
- `ShortcutsPlugin` is consumed from `@editorjs/shortcuts-plugin` (extracted from core by a separate change) and registered by the bundle as a default plugin.
...
- `@editorjs/core`: `src/index.ts` (remove hardcoded `use()` calls + fail-loud validation) and dropped dependencies (`bold`, `italic`, `inline-link`, `paragraph`, `clipboard-plugin`, `collaboration-manager`, `dom-adapters`, `shortcuts-plugin`).
